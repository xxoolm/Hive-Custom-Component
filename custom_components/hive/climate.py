...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    platform.async_register_entity_service(
        ...
        "async_boost_heating_on",
    )

    platform.async_register_entity_service(
        SERVICE_BOOST_HEATING_OFF,
        {},
        "async_boost_heating_off",
    )
...
class HiveClimateEntity(HiveEntity, ClimateEntity):
    ...
    @refresh_system
    async def async_boost_heating_on(self, time_period, temperature):
        """Handle boost heating on service call."""
        await self.hive.heating.setBoostOn(self.device, time_period, temperature)

    @refresh_system
    async def async_boost_heating_off(self):
        """Handle boost heating off service call."""
        await self.hive.heating.setBoostOff(self.device)
